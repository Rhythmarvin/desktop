...
| `0006`  | `user_config` key/value table for typed non-sensitive user preferences.                                                                                                                     |

`default_migration_catalog()` returns all six migrations with every version as the active target.
...
Migration `0006` creates `user_config (key TEXT PRIMARY KEY, value TEXT NOT NULL)`. Its rollback drops only that table. Stable supported keys are `developer_mode` (`true` or `false`) and `log_level` (a canonical lower-case supported level); codecs and defaults remain outside the migration.
