...
Each created, updated, overwritten, or deleted skill is atomic across its SQLite row and `<ORA_DATA_DIR>/atoms/skills/<name>/` package. The package transaction uses same-filesystem staging and backup directories, while import sources remain in OS temp because they may be on a different filesystem. A failed database write rolls the package rename back. Once a transaction commits, cleanup is best-effort: failures are logged and leftover artifacts are reclaimed on the next startup. Startup recovers interrupted transactions and never refuses to start when a visible row lacks a loadable package or root `SKILL.md`; those rows stay in the catalog as unavailable. Unowned directories without a root `SKILL.md` are removed, while untracked complete packages remain in place. Creating or importing an unclaimed name replaces a leftover package through a journaled swap so a failed persist can restore it; creating or importing the same name restores an unavailable row. List and get expose `availability` so pickers can hide unavailable skills while the Skills settings pane can offer delete or re-upload.
