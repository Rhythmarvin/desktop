...
- The backend marks the exact boundary between durable history replay and live provider events.
  The frontend keeps replay staged, publishes the complete history as soon as that boundary
  arrives, and then applies only live-tail updates incrementally. This lets users browse older
  turns while a new turn is still generating. Long transcripts use the shared variable-height
  turn virtualizer, and Workspace-only workflow chats skip task artifact-index construction because
  they do not expose task inline-file links.
